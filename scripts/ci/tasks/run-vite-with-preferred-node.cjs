...
const VITE_CLI_PATH = join(REPO_ROOT, "node_modules", "vite", "bin", "vite.js");
...
function resolvePreferredNode() {
  ...
  throw new Error(
    "Unable to find a Node.js executable compatible with Vite (requires Node 18+).",
  );
}

function main() {
  if (!existsSync(VITE_CLI_PATH)) {
    throw new Error(
      `Unable to find Vite CLI at ${VITE_CLI_PATH}. Run npm ci before invoking UI scripts.`,
    );
  }

  const nodePath = resolvePreferredNode();
  const result = spawnSync(nodePath, [VITE_CLI_PATH, ...process.argv.slice(2)], {
    cwd: process.cwd(),
    env: process.env,
    stdio: "inherit",
  });
