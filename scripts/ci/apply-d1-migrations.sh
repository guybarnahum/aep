...
WRANGLER_CONFIG="${WRANGLER_CONFIG:-core/control-plane/wrangler.jsonc}"
OPERATOR_AGENT_WRANGLER_CONFIG="${OPERATOR_AGENT_WRANGLER_CONFIG:-core/operator-agent/wrangler.jsonc}"
APPLY_OPERATOR_AGENT_MIGRATIONS="${APPLY_OPERATOR_AGENT_MIGRATIONS:-true}"
...
if [[ "${APPLY_OPERATOR_AGENT_MIGRATIONS}" == "true" ]]; then
  : "${OPERATOR_AGENT_D1_DATABASE_NAME:?OPERATOR_AGENT_D1_DATABASE_NAME must be set when APPLY_OPERATOR_AGENT_MIGRATIONS=true}"
  ...
  echo "Skipping D1 migrations (operator-agent)"
fi