...
import type { StartWorkflowRequest, StepName } from "../../../packages/event-schema/src/index";
...
import { auditCleanup, runHealthCheck, runSmokeTest } from "../../../services/proving-ground/src/index";
...
export class WorkflowCoordinatorDO {
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (request.method === "POST" && url.pathname === "/start") {
      ...
      return Response.json({ ok: true, workflow_run_id: current.workflowRunId });
    }
    ...
    if (request.method === "POST" && url.pathname === "/cancel") {
      ...
      await this.state.storage.put("state", current);
      await this.env.DB.prepare(`UPDATE workflow_runs SET cancel_requested = 1 WHERE id = ?`)
  ...
  private async runWorkflow(): Promise<void> {
    ...
    await emitEvent(this.env.DB, {
      ...
      payload: { service_name: current.serviceName, repo_url: current.repoUrl },
    });

    try {
      for (const step of STEPS) {
        if (current.cancelled && !["TEARDOWN", "CLEANUP_AUDIT", "COMPLETE"].includes(step)) {
  ...
  private async runStep(step: StepName, current: State): Promise<void> {
    ...
    try {
      ...
      switch (step) {
        ...
        case "CREATE_ENV":
          ...
          await this.env.DB.prepare(
            ...
            .bind(current.environmentId, current.workflowRunId, current.tenantId, current.projectId, "created", nowIso())
            .run();
          break;
        case "DEPLOY": {
          ...
          await this.env.DB.prepare(
            `INSERT INTO deployments (id, environment_id, deployment_provider, deployment_ref, url, status, created_at)
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
          )
            .bind(current.deploymentId, current.environmentId, result.provider, result.deploymentRef, result.previewUrl, "deployed", nowIso())
            .run();
          await this.env.DB.prepare(`UPDATE environments SET status = ?, preview_url = ? WHERE id = ?`)
          ...
          await emitEvent(this.env.DB, {
            ...
            payload: { url: current.previewUrl, provider: result.provider },
          });
          break;
        }
        case "HEALTH_CHECK":
          ...
          await emitEvent(this.env.DB, {
            ...
            payload: { url: current.previewUrl, expected_status: 200 },
          });
          break;
        case "SMOKE_TEST":
          ...
          await emitEvent(this.env.DB, {
            ...
            payload: { url: current.previewUrl ? `${current.previewUrl}/hello` : null },
          });
          break;
        case "TEARDOWN":
          ...
          if (current.deploymentId) {
            await this.env.DB.prepare(`UPDATE deployments SET status = ?, destroyed_at = ? WHERE id = ?`)
              .bind("destroyed", nowIso(), current.deploymentId)
              .run();
          }
          if (current.environmentId) {
            await this.env.DB.prepare(`UPDATE environments SET status = ?, destroyed_at = ? WHERE id = ?`)
          ...
          await emitEvent(this.env.DB, {
            ...
            payload: { environment_id: current.environmentId, deployment_id: current.deploymentId },
          });
          break;
        case "CLEANUP_AUDIT": {
          const envCheck = await this.env.DB.prepare(`SELECT status FROM environments WHERE id = ?`)
            .bind(current.environmentId)
            .first<{ status: string }>();
          const depCheck = await this.env.DB.prepare(`SELECT status FROM deployments WHERE id = ?`)
          ...
          await emitEvent(this.env.DB, {
            ...
            workflowRunId: current.workflowRunId,
            stepName: step,
            eventType: result.ok ? "cleanup.audit.passed" : "cleanup.audit.failed",
            payload: result,
          });
          if (!result.ok) {
      ...
      await this.env.DB.prepare(
        ...
        .bind("failed", nowIso(), error instanceof Error ? error.message : String(error), stepId)
        .run();
      throw error;
    }
  }

  private async setWorkflowStatus(workflowRunId: string, status: string): Promise<void> {
    const completedAt = ["completed", "failed", "cancelled"].includes(status) ? nowIso() : null;
    await this.env.DB.prepare(`UPDATE workflow_runs SET status = ?, completed_at = ? WHERE id = ?`)
