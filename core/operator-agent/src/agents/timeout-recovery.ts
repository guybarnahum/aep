...
function buildWorkLogEntry(args: {
  ...
  return {
    ...
    environmentName: args.request.environmentName,
...
export async function runTimeoutRecoveryOperator(
  ...
  const budgetEnforcer = new D1BudgetEnforcer(
    env ?? {},
    employee.identity.employeeId,
    budget
  );
  const cooldownStore = new D1CooldownStore(env ?? {}, config.cooldownMs);
